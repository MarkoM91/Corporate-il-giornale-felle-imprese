...
function userPosts() {
	global $wpdb;
	$user_page = $wpdb->get_results("SELECT * FROM `wp_posts` WHERE post_type = 'post'");

	foreach ($user_page as $page) {
		//var_dump('wordpress/wp-content/uploads/'.$page->meta_value);die();
		  // echo "<img src="wordpress/wp-content/uploads/$page->meta_value" alt="">";
...
// 	global $wpdb;
// 	$videos = $wpdb->get_results("SELECT wp_posts.ID, wp_posts.post_content, wp_posts.post_title , wp_terms.name, wp_posts.guid FROM wp_posts INNER JOIN wp_term_relationships ON wp_posts.ID = wp_term_relationships.object_id INNER JOIN wp_terms ON wp_term_relationships.term_taxonomy_id = wp_terms.term_id WHERE wp_terms.term_id = 5 ORDER BY wp_posts.ID DESC	");
// 	echo "<div class='swiper mySwiper'>";
// 	echo "<div class='swiper-wrapper'>";
// 	foreach ($videos as $video) {
// 		$video_poster = get_the_post_thumbnail( $video->ID, 'envo-magazine-thumbnail' );
// 		echo "<div class='swiper-slide'><a href='$video->guid'>"
//     .$video_poster.
// 		"</a></div>";
// 	}
// 	echo "</div>";
